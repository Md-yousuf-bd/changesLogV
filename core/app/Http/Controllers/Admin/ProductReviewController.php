...
class ProductReviewController extends Controller
{
    ...
    {
        $validationMessage = $request->images ? 'nullable' : 'required';

        $request->validate([
            'comment'  => [$validationMessage, 'string', 'max:2000'],
            'images'   => ['nullable', 'array', 'max:10'],
            'images.*' => ['nullable', 'image', new FileTypeValidate(['jpg', 'jpeg', 'png'])],
        ]);

        $review = ProductReview::with(['productReviewReply'])->find($id);
        if (!$review) {
            $notify[] = ['error', 'Invalid review reply'];
            return back()->withNotify($notify);
        }
